...
fn wait_until_rng_ready() -> Result<(), Error> {
    // Check /dev/random to make sure it is ok to read from /dev/urandom.
    //
    // Polling avoids draining the estimated entropy from /dev/random;
    // short-lived processes reading even a single byte from /dev/random
    // could be problematic if they are being executed faster than entropy is
    // collected.
    //
    ...
    // it may be invoked as `SYS_POLL`, `SYS_PPOLL`, or (rarely?) even
    // `SYS_SELECT`, depending on the libc implementation (e.g. glibc vs musl),
    // libc version, potentially the kernel version at runtime, and/or the
    // target architecture.
    //
    ...
    loop {
        ...
        match err.raw_os_error() {
            ...
            Some(libc::ENOSYS) | Some(libc::EPERM) => {
                // Assume a sandbox is blocking the syscall; fall back to
                // reading.
                break;
            }
            _ => return Err(err),
